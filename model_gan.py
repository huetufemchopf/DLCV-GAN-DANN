...
class Discriminator(nn.Module):
    def __init__(self, image_size=64, conv_dim=32):
        super(Discriminator, self).__init__()
        self.conv1 = conv(3, conv_dim, 4, bn=True)
...
class Generator(nn.Module):
    def __init__(self, z_dim=64, image_size=64, conv_dim=32):
    ...
    def forward(self, z):
        ...
        out = self.fc(z)
        out = F.relu(self.deconv1(out), 0.05)
        out = F.relu(self.deconv2(out), 0.05)
        out = F.relu(self.deconv3(out), 0.05)
